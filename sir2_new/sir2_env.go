...
func (ev *SIREnv) SetState() {
	...
	if ev.Act != Recall1 && ev.Act != Recall2 {
		ev.CtrlInput.Values[ev.Act] = 1 // S1, S2, or Ignore gets activated
	} else {
		if ev.OneR { //R gets activated along with the ORIGINAL store ctrl input value
			ev.CtrlInput.Values[3] = 1 //Activates the R
			if ev.Act == Recall1 {
			...
			ev.CtrlInput.Values[ev.Act] = 1 // R1, R2  gets activated
		}
...
func (ev *SIREnv) StepSIR() {
	for {
		...
		if ev.fillstim {
			if ev.Act == Recall1 || ev.Act == Recall2 {
				if ev.Maint1 < 0 || ev.Maint2 < 0 { //one of the stim is empty
					continue
				}
