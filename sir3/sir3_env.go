...
func (ev *SIREnv) String() string {
	...
	return fmt.Sprintf("%v_%v_mnt1_%v_mnt2_%v_mnt3_%v_rew_%v", ev.Act, ev.Stim, ev.Maint1, ev.Maint2, ev.Maint3, ev.Reward.Values[0])
}
...
func (ev *SIREnv) SetState() {
	...
	if ev.Act != Recall1 && ev.Act != Recall2 && ev.Act != Recall3 {
		ev.CtrlInput.Values[ev.Act] = 1 // S1, S2, S3, or Ignore gets activated
	} else {
		if ev.OneR { //R gets activated along with the ORIGINAL store ctrl input value
			ev.CtrlInput.Values[4] = 1 //Activates the R
			if ev.Act == Recall1 {
