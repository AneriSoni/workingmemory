...
func (ev *SIREnv) States() env.Elements {
	els := env.Elements{
		{"Input", []int{ev.NStim}, []string{"N"}},
		{"CtrlInput", []int{int(ActionsN)}, []string{"N"}},
...
//	for {
//		ev.Act = Actions(rand.Intn(int(ActionsN))) //choosing random action
//		
...
func (ev *SIREnv) StepSIR() {
	for {
		...
		// for SIR1 use the following two conditionals..
	//	if ev.Act == Store2 {
	//	ev.Act = Store1
	//	}
	...
	//	}
		...
		if ev.Act == Recall1 && ev.Maint1 < 0 { // nothign
			continue
		}
		if ev.Act == Recall2 && ev.Maint2 < 0 { // nothign
			continue
		}
